...
7. 如果你发现了可复用的模式，更新相关 `AGENTS.md` 文件（见下文）
...
提交前，检查你修改过的文件附近是否有值得沉淀到 `AGENTS.md` 里的经验：

1. **确定被修改的目录**，看看你改了哪些目录
2. **检查是否已有 AGENTS.md**，在这些目录或其父目录中寻找 `AGENTS.md`
...
只有当你掌握了 **确实可复用** 且对该目录后续工作有帮助的知识时，才更新 `AGENTS.md`。
...
对于任何会修改 UI 的 story，如果你配置了浏览器测试工具，就要在浏览器中验证：
